...
PersonSet *PersonSetCreate() {
  ...
  pSet->array = (Person**) malloc(INITIAL_CAPACITY*sizeof(Person *));
...
Person *PersonSetRemove(PersonSet *ps, int id) {
  ...
  // COMPLETE ...
  int indice = search(ps, id);

  if (indice >= 0) {
    Person *p = ps->array[indice];
    ps->array[indice] = ps->array[--(ps->size)];
...
int PersonSetIsSubset(const PersonSet *ps1, const PersonSet *ps2) {
  ...
  for (int i = 0; i < ps1->size; i++) {
    ...
    if (!PersonSetContains(ps2, p->id)) return FALSE;
  }
